...
def main_create_dataset():
    parser = argparse.ArgumentParser(description="Create SEN12FLOOD paired dataset.")
    parser.add_argument("--dataset-root", required=True, type=str, help="Path to SEN12FLOOD ALIGNED dataset root.")
    parser.add_argument("--save-root", required=True, type=str, help="Path to save aligned paired data (train/test).")
    args = parser.parse_args()
    # Define paths
    dataset_root = args.dataset_root
    saving_dir_train = Path(args.save_root) / "train"
    saving_dir_test = Path(args.save_root) / "test"
    ...
    train_csv_path = project_root / "datasets/sen12_train.csv"  # construct the absolute path to the train csv
    test_csv_path = project_root / "datasets/sen12_test.csv"  # construct the absolute path to the test csv
    
    json_file = project_root / "datasets/S1list.json"  # construct the absolute path to the json file
    
    train_log_file = os.path.join(Path(args.save_root), "train_log.txt")
    test_log_file = os.path.join(Path(args.save_root), "test_log.txt")
    ...
    train_dataset = SEN12FloodDataset(dataset_root, json_file, train_csv_path)
    test_dataset = SEN12FloodDataset(dataset_root, json_file, test_csv_path)
...
def main_visualize():
    pair_file_path = r"PATH_2_DATASET\SEN12FLOOD_PAIRS\train\sample__1.npy"  # Replace with your actual path to a saved pair
    visualize_image_pair(pair_file_path)

    # Example Usage
    folder_path = r"PATH_2_DATASET\SEN12FLOOD\0332"  # Replace with your actual folder path
    filename = 'aname'  # Replace with the actual file name without channel suffix
