...
class MixMIM(nn.Module):
    def __init__(self, decoder_dim=512, decoder_depth=8, decoder_num_heads=16, 
        ...
        self.decoder_pred = nn.Linear(
            decoder_dim,
            self.encoder_stride ** 2 * in_chans
        )
    ...
    def forward_loss(self, x, x_rec, mask, mse_linear_weight):
        B, L, C = x_rec.shape
        ...
        mse_linear_weight = self.patchify(mse_linear_weight)
        ...
        loss_rec = mse_linear_weight*((unmix_x_rec - target) ** 2)
        loss_rec = loss_rec.mean()

        return loss_rec

    def forward(self, x, mse_linear_weight, mask_ratio=0.5):
        ...
        loss = self.forward_loss(x, x_rec, mask_s4, mse_linear_weight)
...
def mixmim_base(**kwargs):
    default_args = dict(
        img_size=128, patch_size=4, in_chans=2, num_classes=0, # TODO img_size[224] default
        embed_dim=128, depths=[2, 2, 18, 2], num_heads=[4, 8, 16, 32],
        window_size=[8, 8, 8, 4], mlp_ratio=4, qkv_bias=True, qk_scale=None, # TODO window_size[14, 14, 14, 7] default
...
def mixmim_large(**kwargs):
    default_args = dict(
        img_size=128, patch_size=4, in_chans=2, num_classes=0,
